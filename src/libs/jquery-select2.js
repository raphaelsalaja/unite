	...
	;(Sifter.prototype.tokenize = function (query, respect_word_boundaries) {
		...
		(Sifter.prototype.getScoreFunction = function (search, options) {
			function scoreValue(value, token) {
				var pos
				return !value || -1 === (pos = (value = String(value || '')).search(token.regex))
					? 0
					: ((token = token.string.length / value.length), 0 === pos && (token += 0.5), token)
			}
			var field_count,
	...
	var cmp = function (a, b) {
			return 'number' == typeof a && 'number' == typeof b
				? b < a
					...
					? -1
					: 0
				: ((a = asciifold(String(a || ''))), (b = asciifold(String(b || ''))) < a ? 1 : a < b ? -1 : 0)
		},
		extend = function (a, b) {
							...
							if (((this.plugins = {names: [], settings: {}, requested: {}, loaded: {}}), utils.isArray(plugins)))
								for (i = 0, n = plugins.length; i < n; i++)
									'string' == typeof plugins[i]
										...
										: ((this.plugins.settings[plugins[i].name] = plugins[i].options), queue.push(plugins[i].name))
	...
	(function (root, factory) {
						...
						e.type &&
							...
							46 === keyCode || 8 === keyCode
								...
								: printable &&
								  ...
								  (options += selection = keyCode ? selection.toUpperCase() : selection.toLowerCase()))),
						(e = (printable = $input.attr('placeholder')) ? measureString(printable, $input) + 4 : 0),
						(keyCode = Math.max(measureString(options, $input), e) + 4) !== currentWidth &&
							((currentWidth = keyCode), $input.width(keyCode), $input.triggerHandler('resize')))
			}
		...
		var options,
			...
			Selectize = function ($input, settings) {
				...
				if (
					((computedStyle =
						(computedStyle ? computedStyle.getPropertyValue('direction') : input.currentStyle && input.currentStyle.direction) ||
		...
		return (
			...
			$.extend(Selectize.prototype, {
							...
							case 40:
								return (
									!this.isOpen && this.hasOptions
										? this.open()
										: this.$activeOption &&
										  ((this.ignoreHover = !0), ($next = this.getAdjacentOption(this.$activeOption, 1)).length && this.setActiveOption($next, !0, !0)),
							...
							case 9:
								return (
									this.settings.selectOnTab &&
										this.isOpen &&
										this.$activeOption &&
										(this.onOptionSelect({currentTarget: this.$activeOption}), this.isFull() || e.preventDefault()),
						...
						($target = $(e.currentTarget)).hasClass('create')
							...
							: void 0 !== ($target = $target.attr('data-value')) &&
							  ...
							  self.settings.closeAfterSelect
									? self.close()
									: !self.settings.hideSelected && e.type && /mouse/.test(e.type) && self.setActiveOption(self.getOption($target)))
				},
									...
									(regex = 'string' == typeof pattern ? new RegExp(pattern, 'i') : pattern),
										(highlight = function (node) {
											...
											if (3 === node.nodeType) {
												...
												('highlight' !== node.className || 'SPAN' !== node.tagName)
											)
							...
							? (0 < results.items.length
									? ((query = active_before && self.getOption(active_before)),
									  '' !== results.query && query && query.length
											? ($active = query)
											: 'single' === self.settings.mode && self.items.length && ($active = self.getOption(self.items[0])),
									  ($active && $active.length) ||
											($active =
												$create && !self.settings.addPrecedence ? self.getAdjacentOption($create, 1) : $dropdown_content.find('[data-selectable]:first')))
									: ($active = $create),
				...
				getElementWithTextContent: function (textContent, ignoreCase, $els) {
					if (null != (textContent = hash_key(textContent)))
						for (var i = 0, n = $els.length; i < n; i++) {
							...
							if (
								(1 == ignoreCase && ((eleTextContent = null !== eleTextContent ? eleTextContent.toLowerCase() : null), (textContent = textContent.toLowerCase())),
				...
				createItem: function (input, triggerDropdown) {
					...
					if (('function' != typeof callback && (callback = function () {}), 'boolean' != typeof triggerDropdown && (triggerDropdown = !0), !self.canCreate(input)))
						return callback(), !1
					self.lock()
					var fn,
						called,
				...
				deleteSelection: function (e) {
					...
					if (
						(this.$activeOption &&
							...
							(option_select = (
								'string' == typeof this.settings.deselectBehavior && 'top' === this.settings.deselectBehavior
									? this.getFirstOption()
					...
					0 !== direction &&
						...
						this.isFocused && !this.isInputHidden
							...
							: (selection = this.$control.children('.active:' + tail)).length &&
							  ((valueLength = this.$control.children(':not(input)').index(selection)),
							  this.setActiveItem(null),
			...
			Selectize.define('auto_position', function () {
				...
				this.positionDropdown = function () {
					...
					var dropdownHeight = this.$dropdown.prop('scrollHeight') + 5,
						dropdownHeight =
							this.$control.get(0).getBoundingClientRect().top + dropdownHeight + this.$wrapper.height() > window.innerHeight ? POSITION.top : POSITION.bottom,
			...
			Selectize.define('auto_select_on_type', function (options) {
				...
				self.onBlur =
					...
					function (e) {
						...
						return (
							void 0 !== $matchedItem.attr('data-value') && self.getValue() !== $matchedItem.attr('data-value') && self.setValue($matchedItem.attr('data-value')),
						...
						(this.onKeyDown =
							...
							function (e) {
								...
								;(self.ignoreHover = !0),
									...
									($optgroup = (e = ($optgroup = 37 === e.keyCode ? $optgroup.prev('[data-group]') : $optgroup.next('[data-group]')).find(
										'[data-selectable]'
									)).eq(Math.min(e.length - 1, index))).length && this.setActiveOption($optgroup)
							})),
						function () {
							var div,
										...
										options.append &&
											...
											thisRef.$control.on('click', '.' + options.className, function (e) {
												if ((e.preventDefault(), !self.isLocked))
													return (e = $(e.currentTarget).parent()), self.setActiveItem(e), self.deleteSelection() && self.setCaret(self.items.length), !1
											})
									})
						  })(this, options)
			}),
						...
						function (e) {
							...
							return 8 === e.keyCode && '' === this.$control_input.val() && !this.$activeItems.length && 0 <= (index = this.caretPos - 1) && index < this.items.length
								? ((index = this.options[this.items[index]]),
								  this.deleteSelection(e) && (this.setTextboxValue(options.text.apply(this, [index])), this.refreshOptions(!0)),
								  void e.preventDefault())
								: original.apply(this, arguments)
						}))
			}),
